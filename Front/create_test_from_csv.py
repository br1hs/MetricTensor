#!/usr/bin/env python3
"""
Crear un CSV de prueba (test_input_real.csv) a partir de ../Data/1_cumulative_2025.csv
Muestrea N filas, calcula features faltantes según fórmulas dadas, drop_cols y
genera un CSV con las columnas en el orden que espera el predictor.
"""
...
DATA_PATH = P.parent.joinpath("Data", "1_cumulative_2025.csv")
OUT_PATH = P.joinpath("test_input_real.csv")
...
def main(n_sample=100):
    print("Leyendo CSV:", DATA_PATH)
    if not DATA_PATH.exists():
        raise FileNotFoundError(f"No existe {DATA_PATH}. Ajusta la ruta.")
    df_full = pd.read_csv(DATA_PATH, low_memory=False)
    total_rows = len(df_full)
    print(f"Filas totales en el CSV original: {total_rows}")

    df_full = df_full.drop(columns=[c for c in drop_cols if c in df_full.columns], errors='ignore')

    numeric_cols = df_full.select_dtypes(include=[np.number]).columns.tolist()
    medians = df_full[numeric_cols].median(numeric_only=True).to_dict()

    # sample N filas (si N > total, usa todas)
    if n_sample >= total_rows:
        df_sample = df_full.copy()
        print(f"Usando todas las filas ({total_rows})")
    else:
        df_sample = df_full.sample(n_sample, random_state=42).reset_index(drop=True)
        print(f"Usando muestra de {n_sample} filas")
    ...
    if to_drop:
        print(f"Eliminando columnas sospechosas (drop): {to_drop}")
    ...
    print(f"Archivo guardado: {OUT_PATH} (filas={len(out_df)}, columnas={len(out_df.columns)})")
    print("Primeras filas:")
    print(out_df.head().to_string(index=False))

if __name__ == "__main__":
    parser = argparse.ArgumentParser()
    parser.add_argument("--n", type=int, default=100, help="Número de filas para samplear (default 100). Si n >= filas totales, usa todas.")
    args = parser.parse_args()
    main(n_sample=args.n)
